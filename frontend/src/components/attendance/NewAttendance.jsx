    ...
    return (
        <Box sx={{ position: 'relative', mb: 1.25, borderRadius: 2, overflow: 'hidden', userSelect: 'none' }}>
            ...
            <Box sx={{
                ...
                bgcolor: isAbsent ? (clampedDelta > 0 ? presentColor : absentColor) : (clampedDelta > 0 ? absentColor : presentColor),
...
const NewAttendance = ({ onBack }) => {
    ...
    const markAll = (status) => {
        // Only mark ALL students (including filtered-out ones)
        const updated = {};
        ...
        return (
            <Box sx={{ px: 2.5, py: 3, maxWidth: 480, mx: 'auto' }}>
                ...
                <Button variant="contained" fullWidth
                    disabled={!batch || !stream || loadingStudents} onClick={loadStudents}
            ...
            <Box sx={{
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Button startIcon={<ArrowBackIcon />} onClick={() => setStep('config')}
                            sx={{ minWidth: 0, p: 0.5, color: 'text.secondary' }} />
